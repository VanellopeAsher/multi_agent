...
logging.basicConfig(
    filename=f'logs/mathchat_improved_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log',
    level=logging.INFO,
...
expert = AssistantAgent(
    ...
    Query Requirements:
    1. Output Format: Always utilize the print function for displaying results. Use fractions
    or radical forms instead of decimal numbers.
    ...
    First, articulate the key idea or concept necessary to solve the problem. You can choose
    from the following three approaches:
    Case 1: Direct Python Solution. If the problem can be solved directly using Python code,
    write a program to solve it. Feel free to enumerate all possible arrangements if
    necessary.
    Case 2: Reasoning-Based Solution. If the problem primarily involves reasoning, solve it
    directly without coding.
    Case 3: Step-by-Step Process. If the problem cannot be addressed using the above methods,
    follow this structured approach:
    1. Break down the problem into manageable steps (avoid excessive granularity).
    2. Identify any queries that can be computed using Python (e.g., calculations or equations
    ).
    3. Await my input for any results obtained.
    4. If the results are valid and expected, proceed with your solution. If not, revise your
    query or reasoning accordingly.
    29
    Why Do Multi-Agent LLM Systems Fail?
    Handling Missing Data:
    If a problem is deemed unsolvable due to missing data, return \boxed{’None’}.
    Ensure that only numerical values are placed inside the \boxed{}; any accompanying words
    should be outside.
    Verification Steps:
    Before presenting your final answer, please complete the following steps:
    1. Take a moment to breathe deeply and ensure clarity of thought.
    2. Verify your solution step by step, documenting each part of the verification process in
    a designated VERIFICATION section.
    3. Once you are confident in your verification and certain of your answer, present your
    final result in the format \boxed{_you_answer_}, ensuring only numbers are inside.
...
while processed_count < NUM_SAMPLE:
    for item in samples:
        ...
        # 每次评测后保存结果
        with open("results/mathchat_improved_results.json", "w") as f:
...
print("Results have been saved to results/mathchat_improved_results.json")